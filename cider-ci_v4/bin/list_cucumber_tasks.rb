  ...
  File.open(filepath,'w') do |f|
    ...
    h1.map do |k,v|
      ...
      v.each_slice(CI_SCENARIOS_PER_TASK) do |lines|
        path = ([k] + lines).join(':')
        xvfb = "xvfb-run -a -e log/xvfb.log --server-args='-screen 0 1024x768x24'"
        case test_with
        when :cucumber
...
filepath = 'cider-ci_v4/tasks/manage_scenarios.yml'
...
filepath = 'cider-ci_v4/tasks/manage_problematic_scenarios.yml'
...
filepath = 'cider-ci_v4/tasks/borrow_scenarios.yml'
create_scenario_tasks(filepath, borrow_feature_files_paths, :cucumber)

# keep failing CI scenarios in a separate yml files (and job)
filepath = 'cider-ci_v4/tasks/borrow_problematic_scenarios.yml'
create_scenario_tasks(filepath, borrow_feature_files_paths, :cucumber, tags: ['@problematic'])

############################## ENGINES ##################################

ENGINES.each do |engine|
  filepath = "cider-ci_v4/tasks/#{engine}_scenarios.yml"
